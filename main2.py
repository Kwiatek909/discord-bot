...
# DODAJ DO SEKCJI Z GLOBALNYMI ZMIENNYMI (na górze pliku):
MISTRZOW_FILE = 'mistrzow.json'  # Ścieżka do pliku z danymi mistrzów
all_mistrzow_data = []
last_mistrzow_load_time = 0

# DODAJ DO SEKCJI Z FUNKCJAMI POMOCNICZYMI:

def load_mistrzow_data():
    """Ładuje dane mistrzów z pliku JSON."""
    try:
        with open(MISTRZOW_FILE, 'r', encoding='utf-8') as f:
            return json.load(f)
    except FileNotFoundError:
        print(f"Plik {MISTRZOW_FILE} nie został znaleziony.")
        return []
    except json.JSONDecodeError:
        print(f"Błąd w formacie JSON w pliku {MISTRZOW_FILE}.")
        return []

def get_user_paczki_mistrzow(user_id):
    """Zwraca liczbę paczek Mistrzów użytkownika."""
    user_id_str = str(user_id)
    if user_id_str in bot_data:
        return bot_data[user_id_str].get("paczki_mistrzow", 0)
    return 0

def ensure_user_fields(user_id, user_name):
    """Zapewnia że użytkownik ma wszystkie wymagane pola w danych."""
    user_id_str = str(user_id)
    needs_save = False
    ...
    if user_id_str not in bot_data:
        ...
        needs_save = True
        print(f"Utworzono nowe konto dla użytkownika {user_name}")
    else:
        ...
        for field, default_value in required_fields.items():
            if field not in bot_data[user_id_str]:
                bot_data[user_id_str][field] = default_value
                needs_save = True
                print(f"Dodano brakujące pole '{field}' dla użytkownika {user_name}")
    
    if needs_save:
        save_data(bot_data)
    
    return bot_data[user_id_str]

# DODAJ DO SEKCJI ŁADOWANIA DANYCH NA STARCIE BOTA (w funkcji main lub na początku):

# Ładowanie danych mistrzów przy starcie
if os.path.exists(MISTRZOW_FILE):
    all_mistrzow_data = load_mistrzow_data()
    last_mistrzow_load_time = os.path.getmtime(MISTRZOW_FILE)
    print(f"Załadowano {len(all_mistrzow_data)} mistrzów z {MISTRZOW_FILE}")
else:
    print(f"Plik {MISTRZOW_FILE} nie istnieje. Dane mistrzów będą puste.")
    all_mistrzow_data = []
    last_mistrzow_load_time = 0
...
def migrate_user_data_on_startup():
    """
    Automatycznie dodaje brakujące pole 'paczki_mistrzow' do wszystkich użytkowników przy starcie bota.
    Uruchamia się tylko raz przy każdym starcie.
    """
    global bot_data
    needs_save = False
    updated_count = 0
    
    print("Sprawdzanie i aktualizacja danych użytkowników...")
    
    for user_id, user_data in bot_data.items():
        # Sprawdź czy brakuje pola paczki_mistrzow
        if "paczki_mistrzow" not in user_data:
            user_data["paczki_mistrzow"] = 0
            user_name = user_data.get("user_name", f"User_{user_id}")
            print(f"Dodano paczki_mistrzow dla użytkownika: {user_name}")
            updated_count += 1
            needs_save = True
    
    if needs_save:
        save_data(bot_data)
        print(f"Zaktualizowano {updated_count} kont użytkowników z nowym polem 'paczki_mistrzow'!")
    else:
        print("Wszystkie konta użytkowników już posiadają pole 'paczki_mistrzow'!")

async def main():
    # ... inne inicjalizacje ...
    
    # Załadowanie danych użytkowników
    global bot_data
    bot_data = load_data()
    
    # DODAJ TĘ LINIJKĘ TUTAJ:
    migrate_user_data_on_startup()
    
    # Załadowanie danych kierowców
    global all_drivers_data, last_drivers_load_time
    if os.path.exists(DRIVERS_FILE):
        all_drivers_data = load_drivers_data()
        last_drivers_load_time = os.path.getmtime(DRIVERS_FILE)
        print(f"Załadowano {len(all_drivers_data)} kierowców z {DRIVERS_FILE}")
    
    # Załadowanie danych legend
    global all_legends_data, last_legends_load_time
    if os.path.exists(LEGENDS_FILE):
        all_legends_data = load_legends_data()
        last_legends_load_time = os.path.getmtime(LEGENDS_FILE)
        print(f"Załadowano {len(all_legends_data)} legend z {LEGENDS_FILE}")
    
    # Załadowanie danych mistrzów
    global all_mistrzow_data, last_mistrzow_load_time
    if os.path.exists(MISTRZOW_FILE):
        all_mistrzow_data = load_mistrzow_data()
        last_mistrzow_load_time = os.path.getmtime(MISTRZOW_FILE)
        print(f"Załadowano {len(all_mistrzow_data)} mistrzów z {MISTRZOW_FILE}")
...
async def sklep_command(interaction: discord.Interaction):
    ...
    buy_mistrzow_pack_button = discord.ui.Button(
        label="Kup paczkę Mistrzów OPRF",
        style=discord.ButtonStyle.danger,
        emoji="🏆",
        custom_id="zakuppaczkamistrzow"
    )

    view = discord.ui.View(timeout=180)
    view.add_item(buy_pack_button)
    view.add_item(buy_legend_pack_button)
    view.add_item(buy_mistrzow_pack_button)
    ...
    async def buy_mistrzow_pack_button_callback(interaction: discord.Interaction):
        user_id = interaction.user.id
        user_display_name = interaction.user.display_name
        ...
        if current_coins >= item_cost:
            ...
            current_paczki_mistrzow = get_user_paczki_mistrzow(user_id) 
            
            bot_data[user_id_str]["oprf_coins"] -= item_cost
            bot_data[user_id_str]["paczki_mistrzow"] += 1
            
            save_data(bot_data)

            await interaction.response.send_message(
                f"**Gratulacje, {user_display_name}!** Pomyślnie zakupiono paczkę Mistrzów OPRF. "
                f"Masz teraz `{bot_data[user_id_str]['oprf_coins']}` OPRF Coinsów i `{bot_data[user_id_str]['paczki_mistrzow']}` paczek mistrzów.",
                ephemeral=True
            )
        else:
            await interaction.response.send_message(
                f"**{user_display_name}**, masz tylko `{current_coins}` OPRF Coinsów. Potrzebujesz `{item_cost}` OPRF Coinsów, aby kupić paczkę mistrzów.",
                ephemeral=True
            )

    buy_pack_button.callback = buy_pack_button_callback
    buy_legend_pack_button.callback = buy_legend_pack_button_callback
    buy_mistrzow_pack_button.callback = buy_mistrzow_pack_button_callback
...
async def konto_command(interaction: discord.Interaction, member: discord.Member = None):
    ...
    # DODANE: Sprawdź i przeładuj dane użytkowników (bot_data) - analogicznie jak w ranking
    global bot_data, last_data_reload_time, all_drivers_data, last_drivers_load_time, all_legends_data, last_legends_load_time, all_mistrzow_data, last_mistrzow_load_time
    ...
    # Bezpieczna inicjalizacja/aktualizacja danych użytkownika
    user_data = ensure_user_fields(user_id, target_user.name)
    coins = user_data.get("oprf_coins", 0)
    paczki = user_data.get("paczki", 0)
    paczki_legend = user_data.get("paczki_legend", 0)
    paczki_mistrzow = user_data.get("paczki_mistrzow", 0)
    karty = user_data.get("karty", [])
    
    # Sprawdź i przeładuj dane kierowców, legend i mistrzów
    if os.path.exists(DRIVERS_FILE):
    ...
    if karty:
        ...
        for karta in karty:
            ...
            # Jeśli nie znaleziono w legendach, sprawdź mistrzów
            if not driver_data:
                for mistrz in all_mistrzow_data:
                    if mistrz['kierowca'] == karta:
                        driver_data = mistrz
                        break
    ...
    description_text = (
        ...
        f"- Paczki Mistrzów: `{paczki_mistrzow}`\n"
...
async def paczka_command(interaction: discord.Interaction, typ: str):
    ...
    if typ == "kierowcy":
        ...
        pack_image = "https://cdn.discordapp.com/attachments/1268199863636594788/1404607231399559168/obraz_2025-08-12_012723233_preview_rev_1.png?ex=689bcdec&is=689a7c6c&hm=d94c30ff828f19e056bcd1e348564645cfe3c60a3e6f0a2f33018cb60b38893c&"
    else:  # mistrzow
        current_paczki = get_user_paczki_mistrzow(user_id)
        pack_type_display = "Mistrzów"
        pack_image = "https://example.com/mistrzow_pack.png"  # Ustaw swój obraz dla paczki Mistrzów
    ...
    open_pack_button = discord.ui.Button(
        ...
        style=discord.ButtonStyle.success,
        emoji="📦" if typ == "kierowcy" else "✨" if typ == "legend" else "🏆",
    ...
    async def open_pack_button_callback(interaction: discord.Interaction):
        ...
        if typ == "kierowcy":
            ...
            current_paczki_after_check = get_user_paczki_mistrzow(user_id)
...
async def ranking_command(interaction: discord.Interaction):
    ...
    # Upewnij się, że dane mistrzów są aktualne
    if os.path.exists(MISTRZOW_FILE):
        current_mistrzow_mtime = os.path.getmtime(MISTRZOW_FILE)
        if current_mistrzow_mtime > last_mistrzow_load_time:
            all_mistrzow_data = load_mistrzow_data()
    ...
    mistrzow_pack_value = 100  # Możesz dostosować wartość
    card_value = 25
    ...
    for user_id_str, user_data in bot_data.items():
        ...
        total_value = coins + (paczki * kierowcy_pack_value) + (paczki_legend * legend_pack_value) + (paczki_mistrzow * mistrzow_pack_value) + (len(karty) * card_value)
        ...
        account_values.append({
            ...
            "paczki_mistrzow": paczki_mistrzow,
    ...
    for i, user in enumerate(sorted_accounts[:10]):
        ...
        description_lines.append(
            f"**{i + 1}**. {user_display} - `{user['paczki']}` paczek kierowców, `{user['paczki_legend']}` paczek legend, `{user['paczki_mistrzow']}` paczek mistrzów, `{user['oprf_coins']}` OPRF Coinsów, Karty: {user['karty_display']}"
        )
...
async def karta_command(interaction: discord.Interaction, nazwa: str):
    """
    Wyświetla szczegóły karty kierowcy/legendy/mistrza, jeśli użytkownik ją posiada.
    """
    user_id = interaction.user.id
    user_id_str = str(user_id)
    ...
    # Przeładuj dane kierowców, legend i mistrzów, jeśli są nieaktualne
    ...
    # Znajdź dane kierowcy w globalnych listach
    driver_data = None
    card_type = "Kierowcy"
    ...
    if not driver_data:
        for mistrz in all_mistrzow_data:
            if mistrz['kierowca'].lower() == nazwa_lower:
                driver_data = mistrz
                card_type = "Mistrzów"
                break
...
async def lista_paczka_command(interaction: discord.Interaction, typ: str):
    """
    Wyświetla ranking kierowców/legend/mistrzów dostępnych w paczkach, posortowanych po ocenie ogólnej.
    """
    global all_drivers_data, last_drivers_load_time, all_legends_data, last_legends_load_time, all_mistrzow_data, last_mistrzow_load_time
